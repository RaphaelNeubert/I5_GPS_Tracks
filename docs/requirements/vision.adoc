...
An der HTW Dresden wird im Modul Software Engineering 1 eine Projektarbeit gefordert, bei welcher ein zusammengestelltes Team aus Studenten einen Auftrag erfüllen müssen. Ein Professor ist auf diese Projektarbeit zugekommen und möchte für seine mobilen Roboter eine Smartphone App erstellt haben, welche durch GPS-Tracks den mobilen Roboter auf der Weltkarte orten kann, um dem mobilen Roboter per App an bestimmten Orten Aufgaben zuteilen zu können. Da die meißten GPS-Track Apps nur sehr ungenaues GPS-Signal liefern und es dabei nicht möglich ist, bestimmte Punkte für die Aufgabenverteilung an den Roboter auszuwählen, erhofft sich der Professor, dass der mobile Roboter durch die App, auch aus großer Entfernung vom Nutzer benutzt werden kann.
...
[cols="1,3"]
|===
| Das Problem | Die mobilen Roboter können nicht präzise geortet werden und es können keine Aufgaben an die Roboter verteilt werden
...
|===
| Für | Benutzer
| die | mit dem mobilen Roboter arbeiten
...
| Im Gegensatz zu | sehr ungenauen GPS-Track Apps ohne Möglichkeit zur Aufgabenverteilung an den mobilen Roboter
| Unser Produkt | unterstützt die präszise Erstellung von GPS-Tracks und die Aufgabenvertreilung an den Roboter an gewählten Punkten auf der Karte
|===
...
| nimmt GPS-Track Daten auf und verteilt Aufgaben an den mobilen Roboter an gewählten Punkten auf der Karte

| Serverbetreiber
| synchronisiert Daten
| Server muss immer betriebsfähig sein und Daten synchronisieren

| Kartendaten Provider
| synchronisiert Kartendaten
...
| Google Android API
| ortet die Position
| stellt sicher, das die Position des mobilen Roboters aktuell ist
|===
