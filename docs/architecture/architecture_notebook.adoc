...
Das System besteht aus einer Android App für das erstellen von GPS-Tracks und einem Server der aufgenommene
...
Personen (<10) parallel genutzt werden
aber nahezu rund um die Uhr verfügbar sein. Über die verwendeten Endgeräte ist nur bekannt es sich um Android Smartphones
handelt die über ein Touchscreen bedient werden sollen. 
...
* Auf dem Smartphone des Nutzers ist mindestens Android 5.0 oder neuer installiert.
* Jeder Nutzer besitzt ein stabile GPS-Verbindung.
* Jeder Nutzer besitzt eine meist vorhandene Internetverbindung. 
* Der Nutzer hat die Möglichkeit sich mit dem selben Netzwerk mit dem der Server verbunden ist, zu verbinden.
* Der Nutzer hat der App alle benötigten Berechtigungen erteilt.
...
. Speichern eines GPS-Tracks +
    Die GPS-Tracks werden auf dem Endgerät des Nutzers gespeichert und, sofern man eine Verbindung zum Netzwerk des Servers
    hat, mit einer dafür geeigneten Datenbank mit dem Server synchronisiert. Bei der Synchronisations handelt es sich (zunächst) um eine 1 zu 1
    Synchronisation. Nutzeraccounts bzw. eine Nutzerverwaltung und gesonderte Zugriffsrechte soll es zunächst nicht geben.
    Die Synchronisation mit dem Server ermöglicht es leicht dem Roboter Tracks zuzuweisen, unabhängig auf welchem Gerät
    sie aufgenommen oder bearbeitet wurden.
. Löschen eines GPS-Tracks +
    Wenn der Nutzer einen GPS-Track, in der GPS-Trackauswahl, löschen möchte, dann wird ein kleines Pop-up geöffnet, in welchem eine erneute Bestätigung zum löschen angefragt wird. Bei erfolgreicher Bestätigung wird der Track sowohl auf dem
    Endgerät als auch auf dem Server gelöscht.
